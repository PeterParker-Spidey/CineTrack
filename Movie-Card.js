...
async function fetchItemDetails() {
  try {
    const response = await fetch(`${BASE_URL}/movie/${itemId}?api_key=${API_KEY}`);
    if (!response.ok) throw new Error(`Error fetching movie details: ${response.status}`);
    ...
    releaseYearElement.textContent = `Release Year: ${data.release_date ? new Date(data.release_date).getFullYear() : 'N/A'}`;
...
async function fetchStreamingProviders(itemId) {
  try {
    const response = await fetch(`${BASE_URL}/movie/${itemId}/watch/providers?api_key=${API_KEY}`);
    if (!response.ok) {
    ...
    if (Object.keys(providersByCountry).length > 0) {
      ...
      streamingContainer.innerHTML = "<p>Not available for streaming</p>";
    }
  } catch (error) {
    console.error("Error fetching streaming providers:", error);
    streamingContainer.innerHTML = `<p>Something went wrong: ${error.message}. Please try again later.</p>`;
  }
}
...
async function fetchTrailer() {
  const response = await fetch(`${BASE_URL}/movie/${itemId}/videos?api_key=${API_KEY}`);
...
async function fetchCast() {
  try {
    const response = await fetch(`${BASE_URL}/movie/${itemId}/credits?api_key=${API_KEY}`);
...
async function fetchReviews() {
  try {
    const response = await fetch(`${BASE_URL}/movie/${itemId}/reviews?api_key=${API_KEY}`);
